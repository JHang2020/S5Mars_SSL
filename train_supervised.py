    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        if "cityscapes" in cfg["dataset"].get("type", "pascal"):
            if epoch % 10 == 0 or epoch > (cfg_trainer["epochs"]-50):
                ...
                prec_tea = -1000.0
                prec = prec_tea
        elif epoch%10==0 or epoch > (cfg_trainer["epochs"]-60) or epoch==last_epoch:
            if cfg_trainer.get("evaluate_student", True):
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if 1:
            ...
            # 2. apply adain
            trigger_prob = -1.#cfg["trainer"]["unsupervised"].get("use_cutmix_trigger_prob", 1.0)
            if np.random.uniform(0, 1) < trigger_prob: #and cfg["trainer"]["unsupervised"].get("use_cutmix", False):
                image_u_aug = img_aug_adain(image_u_aug)
            ...
            #print(((logits_u_aug>0.95)*(label_u_aug!=255)).float().mean(), pseduo_high_ratio)
